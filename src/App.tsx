...
interface DragOperation<ItemData> {
  placement: {
    x: number;
    y: number;
    width?: number;
...
interface DndInsertOperation<ItemData> {
  placement: {
    x: number;
    y: number;
  };
...
interface ItemRemovalOperation<ItemData> {
  item: BoardProps.Item<ItemData>;
  disturbed: readonly BoardProps.Item<ItemData>[];
}

export default () => {
  const [items, setItems] = React.useState<BoardProps.Item<ItemData>[]>([
    {
      id: '1',
      rowSpan: 1,
      columnSpan: 2,
  ...
  return (
    <Board<ItemData>
      items={items}
      renderItem={(item: BoardProps.Item<ItemData>, actions) => (
        <BoardItem
      ...
      i18nStrings={{
        ...
        liveAnnouncementDndItemReordered: (
          operation: DragOperation<ItemData>,
        ) => {
        ...
        liveAnnouncementDndItemInserted: (
          operation: DndInsertOperation<ItemData>,
        ) => {
          const columns = `column ${operation.placement.x + 1}`;
          const rows = `row ${operation.placement.y + 1}`;
          return createAnnouncement(
        ...
        liveAnnouncementItemRemoved: (op: ItemRemovalOperation<ItemData>) =>
          createAnnouncement(
        ...
        navigationItemAriaLabel: (
          item: BoardProps.Item<ItemData> | null | undefined,
